@props ([
    'fieldType' => ['Input', 'Textarea', 'Image', 'Editor', 'Color'],
    'fields',
    'model',
    'first' => null
])
    ...
    <div class="row">
        <div class="col-md-4">
            @foreach($fieldType as $type)
                <x-template.field-button :label="$type" :id="$model->id"/>
            @endforeach
        </div>
        <div class="col-md-8" id="fieldsWrapper{{ $model->id }}" style="border: 2px grey solid; border-radius: 5px; padding: 5px">
                    ...
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            <button type="button" class="btn btn-tool" data-card-widget="remove"
                                    onclick=removeFields(`{{ $loop->index }}`)>
...
<script>
    function addFields(type, label, id) {
